...
class TestFleetVehicleAnalyticPurchase(TransactionCase):
    ...
    def test_search_purchase_order_by_analytic_account_id(self):
        order = self._create_purchase_order({str(self.account.id): 100})
        other_order = self._create_purchase_order({str(self.other_account.id): 100})
    ...
    def test_search_purchase_order_line_by_analytic_account_id(self):
        order = self._create_purchase_order({str(self.account.id): 100})
        other_order = self._create_purchase_order({str(self.other_account.id): 100})
        lines = self.env["purchase.order.line"].search(
            [("analytic_account_id", "=", self.account.id)]
        )
        self.assertEqual(lines, order.order_line)
        self.assertNotIn(other_order.order_line, lines)
