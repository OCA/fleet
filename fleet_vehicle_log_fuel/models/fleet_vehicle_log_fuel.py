...
class FleetVehicleLogFuel(models.Model):
    ...
    def _onchange_liter_price_amount(self):
        liter = float(self.liter or 0.0)
        price_per_liter = float(self.price_per_liter or 0.0)
    ...
    def button_running(self):
        self.filtered(lambda x: x.state == "todo").write({"state": "running"})
        return True
