...
fuel_costs AS(
    ...
    CROSS JOIN generate_series((
            SELECT
                min(date)
                FROM fleet_vehicle_log_fuel), CURRENT_DATE + '1 month'::interval, '1 month') d
    ...
    SELECT
        ...
        (COALESCE(sum(co.amount), 0)
          + COALESCE(sum(cod.cost_generated
            * extract(day
              FROM least (date_trunc('month', d) + interval '1 month', cod.expiration_date)
              - greatest (date_trunc('month', d), cod.start_date))), 0)
              + COALESCE(sum(com.cost_generated), 0)
              + COALESCE(sum(coy.cost_generated), 0)) AS
        COST,
